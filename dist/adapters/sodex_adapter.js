...
class SodexAdapter {
    ...
    lastNonce = 0;
    wallet;
    constructor(apiKey, apiSecret, userAddress) {
        this.apiKey = apiKey;
        this.apiSecret = apiSecret;
        this.userAddress = userAddress;
        this.wallet = new ethers_1.ethers.Wallet(apiSecret);
    }
    async getSignature(nonce, method, paramsStr) {
        ...
        const payloadHash = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes(payloadStr));
    ...
    async request(method, endpoint, data) {
        ...
        if (nonceVal <= this.lastNonce)
            nonceVal = this.lastNonce + 1;
        ...
        if (data) {
            if (typeof data === 'string') {
                ...
                const cancels = await Promise.all(data.map(async (o) => {
                    const symId = await this.getSymbolId(o.symbol);
                    const orderId = o.orderId.toString();
                    // Use clOrdID for client-assigned IDs (ext-...), orderID for exchange-assigned numeric IDs
                    const isClientId = orderId.startsWith('ext-') || isNaN(Number(orderId));
                    if (isClientId) {
                        return `{"symbolID":${symId},"clOrdID":"${orderId}"}`;
                    }
                    else {
                        const numericId = orderId.startsWith('0x') ? BigInt(orderId).toString() : orderId;
                        return `{"symbolID":${symId},"orderID":${numericId}}`;
                    }
                }));
    ...
    async getAccountId() {
        ...
        try {
            ...
            if (data?.aid) {
                this.cachedAccountId = data.aid;
                return data.aid;
            }
            if (data?.data?.aid) {
                this.cachedAccountId = data.data.aid;
    ...
    async getSymbolId(symbol) {
        if (this.cachedSymbolId[symbol])
            return this.cachedSymbolId[symbol];
        try {
            const data = await this.request('GET', '/markets/symbols');
            const arr = Array.isArray(data) ? data : (data?.data || []);
            const item = arr.find((s) => s.symbol === symbol || s.name === symbol || s.id === symbol);
    ...
    async place_limit_order(symbol, side, price, size, reduceOnly = false, timeInForce = 4) {
    ...
    async get_balance() {
        ...
        const usdt = arr.find((b) => b.asset === 'USDT' || b.currency === 'USDT' || b.coin === 'USDT' || b.coin === 'vUSDC');
        if (usdt) {
...
exports.SodexAdapter = SodexAdapter;
