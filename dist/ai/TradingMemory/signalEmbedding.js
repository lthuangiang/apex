...
exports.signalToEmbedding = signalToEmbedding;
 ...
 */
function signalToEmbedding(signal) {
 ...
 */
function buildPrompt(signal, similarTrades) {
    const signalStr = [
        `price=${signal.price}`,
        `sma50=${signal.sma50}`,
 ...
 */
function parseLLMResponse(raw, winRateOfSimilar) {
    const fallback = {
        direction: 'skip',
        confidence: 0,
    ...
    try {
        ...
        if (direction !== 'long' && direction !== 'short' && direction !== 'skip')
            return fallback;
