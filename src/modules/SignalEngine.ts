...
export class SignalEngine {
    constructor(private adapter: ExchangeAdapter) { }
    ...
    async getSignal(symbol: string): Promise<Signal> {
        try {
            ...
            if (chart) {
                ...
                if (chart.isGreen) chartScore -= 0.1; else chartScore += 0.1; // Green candle = overbought → lower score
                if (chart.tailBullish) chartScore += 0.2;  // Hammer = bounce signal → LONG
                if (chart.tailBearish) chartScore -= 0.2;  // Shooting star = rejection → SHORT
            ...
            if (chart) {
                ...
                if (chart.price > chart.avgClose * 1.002) regime = 'TREND_UP';
                else if (chart.price < chart.avgClose * 0.998) regime = 'TREND_DOWN';
            }
            ...
            const [neutralMin, neutralMax] = config.NEUTRAL_ZONE;

            if (score > neutralMax) direction = 'long';
            else if (score < neutralMin) direction = 'short';
            else direction = 'skip';
            // 8. Tính Confidence
